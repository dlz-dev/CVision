...
for filename, cv_text in all_cvs.items():
    ...
    try:
        ...
        # Le LLM s'occupe des champs complexes (géré avec retries)
        ...
        # Fusion des métadonnées temporelles calculées côté python pour l'éducation
        if education_data:
        ...
        output_path = Path(f"data/extracted/{cv_id}.json")
        ...
        # Toute erreur fatale bascule le fichier en quarantaine (y compris JSONDecodeError propulsé depuis extract_cv)
        print(f"Erreur inattendue pour {filename} : {e}. Déplacement en quarantaine.")
        Path(f"data/quarantaine/{filename}").write_text(cv_text, encoding="utf-8")
        continue
...
try:
    # Lecture optimisée : seules les colonnes requises sont chargées et renommées à la volée
    df_labels = pd.read_csv("data/student_labels.csv", usecols=['filename', 'passed_next_stage']) \
        .rename(columns={'filename': 'cv_id'})

    df_final = pd.merge(df_cvs, df_labels, on="cv_id", how="left")
    df_final.to_csv("data/cv_dataset.csv", index=False, encoding="utf-8")

    print("Pipeline achevé. Fichier final 'cv_dataset.csv' généré.")
except Exception as e:
