...
def process_single_cv(cv: dict, reverse_mapping: dict, base_lang_dict: dict) -> dict:
    """Traite un dictionnaire JSON représentant un CV et retourne un dictionnaire aplati (une ligne)."""
    meta = cv.get("meta", {})
...
def json2csv(input_path_json: str) -> pd.DataFrame:
    ...
    for f in files:
        ...
        row = process_single_cv(cv, REVERSE_MAPPING, BASE_LANG_DICT)
        data.append(row)
